		...
		<div class="admin-content">
    		...
    		<div class="am-cf am-padding">
				<div class="am-fl am-cf">
      				<small>位置</small>：<small>员工管理中心</small>/<small>工资记录</small>
      			</div>
			</div>
    		<form action="${pageContext.request.contextPath}/system/employee/salaryList" method="post" id="form1" class="am-form">
    		<div class="am-g">
    				<div class="am-u-lg-3"> 
					    <div class="am-input-group am-input-group-secondary am-form-group">
					      <span class="am-input-group-label">开始</span>
					      <input type="text" class="am-form-field" placeholder="发放时间/开始时间" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" name="startTime" value="${dateEntity.startTime}" />
					    </div>
					 </div>
					 <div class="am-u-lg-3">
					    <div class="am-input-group am-input-group-secondary am-form-group">
					      <span class="am-input-group-label">结束</span>
					      <input type="text" class="am-form-field" placeholder="发放时间/结束时间" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" name="endTime" value="${dateEntity.endTime}" />
					    </div>
					 </div>
					 <div class="am-u-lg-1">
    		...
    		<form action="${pageContext.request.contextPath}/system/employee/salaryList" method="post" id="form2">
							...
							<tbody>
								<c:forEach items="${list }" var="p" varStatus="status">
									<tr>
										...
										<td><fmt:formatDate value="${p.grantTime}" pattern="yyyy-MM-dd"/></td>
										<td>${p.insuranceMoney}</td>
										<td>${p.performance}</td>
										<td>${p.taxMoney}</td>
										<td>${p.leaveMoney}</td>
										<td>${p.otherMoney}</td>
