	...
	export default({
		...
		mounted(){
			...
			editor.customConfig = {
				...
				uploadImgServer : 'http://10.203.1.87:3000/ueditor/wangeditor/upload' ,
			...
			editor.customConfig.uploadFileName = 'myFileName'  ;//设置上传图片文件的时候，后台接受的文件名，files.myFileName;
			
			editor.customConfig.withCredentials = true ; //跨域上传中如果需要传递 cookie 需设置 withCredentials
			editor.customConfig.uploadImgTimeout = 3000 ; //自定义 timeout 时间，这里是设置的3秒
			editor.customConfig.uploadImgServer = 'http://10.203.1.87:3000/ueditor/wangeditor/upload'  ;//上传到后台的接口
			*/
